...
대상 모델: gpt-4o-mini / 언어: 한국어 / 출처: 연합뉴스TV
...
ENRICH_SYSTEM_PROMPT = """당신은 한국어 뉴스를 분석하는 리서치 애널리스트입니다. 연합뉴스TV 기사 한 건의 제목과 본문을 입력받아, 검색·집계·시각화에 사용할 구조화된 메타데이터를 생성합니다.
...
- sentiment: 기사가 다루는 "주요 대상(main subject)"에 대한 기사의 논조(tone)를 판단합니다. 사건의 사회적 좋고 나쁨이 아니라, 기사가 그 대상을 어떤 논조로 서술하는지를 봅니다.
  ...
  판단이 애매하면 기본값은 neu입니다. 연합뉴스TV 스트레이트 기사는 대체로 neu입니다.
...
ENRICH_USER_TEMPLATE = """다음은 연합뉴스TV 기사입니다. 지정된 JSON 스키마에 맞춰 분석 결과만 출력하세요.
...
CLUSTER_USER_TEMPLATE = """아래는 같은 군집으로 묶인 연합뉴스TV 기사들의 제목과 요약입니다.
